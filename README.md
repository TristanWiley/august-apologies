...
- The Twitch Extension build lives under `extensions/twitch`. Run `npm run build:twitch` to emit the compiled `config.html` and `panel.html` files into `extensions/twitch/dist`.
- For local development, run `bun run dev:twitch` and open `http://localhost:5173/config.html` or `http://localhost:5173/panel.html`.
